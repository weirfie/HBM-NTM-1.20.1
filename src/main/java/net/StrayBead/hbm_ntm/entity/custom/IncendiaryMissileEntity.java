...
public class IncendiaryMissileEntity extends AbstractArrow implements ItemSupplier {
    private static final EntityDataAccessor<Boolean> ACTIVATED = SynchedEntityData.defineId(IncendiaryMissileEntity.class, EntityDataSerializers.BOOLEAN);
    private static final EntityDataAccessor<BlockPos> TARGET_POS = SynchedEntityData.defineId(IncendiaryMissileEntity.class, EntityDataSerializers.BLOCK_POS);
    ...
    public IncendiaryMissileEntity(EntityType<? extends AbstractArrow> p_36721_, Level p_36722_) {
        super(p_36721_, p_36722_);
    }

    public IncendiaryMissileEntity(PlayMessages.SpawnEntity packet, Level world) {
        super(ModEntities.INCENDIARY_MISSILE_ENTITY.get(), world);
    }

    public IncendiaryMissileEntity(EntityType<? extends IncendiaryMissileEntity> type, LivingEntity entity, Level world) {
    ...
    private void handleMissileLogic() {
        ...
        double distToTargetY = this.getY() - targetPos.getY();
        if (flightTicks > 40 && distToTargetY < 40.0D && this.getDeltaMovement().y < 0) {
            deploySubmunitions();
            return;
        }
    ...
    private void deploySubmunitions() {
        if (this.level().isClientSide()) return;

        int submunitionCount = 60;
        ServerLevel serverLevel = (ServerLevel) this.level();

        for (int i = 0; i < submunitionCount; i++) {
            SubmunitionProjectileEntity sub = new SubmunitionProjectileEntity(
                    ModEntities.SUBMUNITION_PROJECTILE.get(),
                    serverLevel
            );

            double startX = this.getX() + (random.nextDouble() - 0.5) * 0.5;
            double startY = this.getY() - 1.0;
            double startZ = this.getZ() + (random.nextDouble() - 0.5) * 0.5;
            sub.moveTo(startX, startY, startZ, this.getYRot(), 0);

            double spread = 0.9D;
            Vec3 velocity = new Vec3(
                    (random.nextDouble() - 0.5) * spread,
                    -0.4D - (random.nextDouble() * 0.8D),
                    (random.nextDouble() - 0.5) * spread
            );

            sub.setDeltaMovement(velocity);
            serverLevel.addFreshEntity(sub);
        }

        serverLevel.playSound(null, this.getX(), this.getY(), this.getZ(),
                net.minecraft.sounds.SoundEvents.GENERIC_EXPLODE,
                net.minecraft.sounds.SoundSource.AMBIENT, 1.5f, 1.2f);
    ...
    public static IncendiaryMissileEntity shoot(LivingEntity entity, LivingEntity target) {
        IncendiaryMissileEntity missile = new IncendiaryMissileEntity(ModEntities.INCENDIARY_MISSILE_ENTITY.get(), entity, entity.level());
