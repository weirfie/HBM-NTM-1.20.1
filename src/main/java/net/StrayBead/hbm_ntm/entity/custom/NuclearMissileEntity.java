...
public class NuclearMissileEntity extends AbstractArrow implements ItemSupplier {
    private static final EntityDataAccessor<Boolean> ACTIVATED = SynchedEntityData.defineId(NuclearMissileEntity.class, EntityDataSerializers.BOOLEAN);
    private static final EntityDataAccessor<BlockPos> TARGET_POS = SynchedEntityData.defineId(NuclearMissileEntity.class, EntityDataSerializers.BLOCK_POS);
    ...
    public NuclearMissileEntity(EntityType<? extends AbstractArrow> p_36721_, Level p_36722_) {
        super(p_36721_, p_36722_);
    }

    public NuclearMissileEntity(PlayMessages.SpawnEntity packet, Level world) {
        super(ModEntities.NUCLEAR_MISSILE_ENTITY.get(), world);
    }

    public NuclearMissileEntity(EntityType<? extends NuclearMissileEntity> type, LivingEntity entity, Level world) {
    ...
    protected void onHitBlock(BlockHitResult hitResult) {
        if (!this.level().isClientSide()) {
            if (this.level() instanceof ServerLevel serverLevel) {
                serverLevel.setBlock(this.blockPosition(), ModBlocks.LITTLE_BOY.get().defaultBlockState(), 3);
    ...
    public static NuclearMissileEntity shoot(LivingEntity entity, LivingEntity target) {
        NuclearMissileEntity missile = new NuclearMissileEntity(ModEntities.NUCLEAR_MISSILE_ENTITY.get(), entity, entity.level());
