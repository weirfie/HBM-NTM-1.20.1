...
public class NuclearParticleManager {

    private static final ResourceLocation CLOUD_TEXTURE = new ResourceLocation("hbm_ntm", "textures/particle/particle_explosion.png");
    ...
    public static void renderSquare(RenderLevelStageEvent event) {
        ...
        synchronized (PARTICLES) {
            ...
            for (int i = 0; i < PARTICLES.size(); i++) {
                ...
                float lifeFactor = (float) data.age / (float) data.maxAge;
                int frame = Math.min((int) (lifeFactor * totalFrames), totalFrames - 1);

                int column = frame % columns;
                int row = frame / columns;

                float uMin = (float) column / columns + eps;
                float uMax = (float) (column + 1) / columns - eps;
                float vMin = (float) row / rows + eps;
                float vMax = (float) (row + 1) / rows - eps;
                ...
                bf.vertex(mat, -s, -s, 0).color(r, g, b, a).uv(uMin, vMax).endVertex();
                bf.vertex(mat,  s, -s, 0).color(r, g, b, a).uv(uMax, vMax).endVertex();
                bf.vertex(mat,  s,  s, 0).color(r, g, b, a).uv(uMax, vMin).endVertex();
                bf.vertex(mat, -s,  s, 0).color(r, g, b, a).uv(uMin, vMin).endVertex();
