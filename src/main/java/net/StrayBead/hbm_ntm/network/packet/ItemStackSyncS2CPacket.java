...
public class ItemStackSyncS2CPacket {
    ...
    public boolean handle(Supplier<NetworkEvent.Context> supplier) {
        ...
        context.enqueueWork(() -> {
            if(Minecraft.getInstance().level.getBlockEntity(pos) instanceof BatterySocketBlockEntity blockEntity) {
                blockEntity.setHandler(this.itemStackHandler);
            }
            if(Minecraft.getInstance().level.getBlockEntity(pos) instanceof BurnerPressBlockEntity blockEntity) {
                blockEntity.setHandler(this.itemStackHandler);
            }
        });
        return true;
