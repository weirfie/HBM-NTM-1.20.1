...
public class RadiolysisChamberScreen extends AbstractContainerScreen<RadiolysisChamberMenu> {
    private final static HashMap<String, Object> guistate = RadiolysisChamberMenu.guistate;
    private EnergyInfoArea energyInfoArea;
    private FluidTankRenderer renderer;
    private FluidTankRenderer outputRenderer;

    public RadiolysisChamberScreen(RadiolysisChamberMenu container, Inventory inventory, Component text) {
        super(container, inventory, text);
        this.imageWidth = 176;
        this.imageHeight = 214;
    }

    private static final ResourceLocation texture = new ResourceLocation("hbm_ntm:textures/screens/gui_radiolysis.png");
    ...
    protected void renderBg(GuiGraphics guiGraphics, float partialTicks, int gx, int gy) {
        ...
        renderer.render(guiGraphics, x + 61, y + 8, menu.getFluidStack());
    ...
    protected void renderLabels(GuiGraphics guiGraphics, int mouseX, int mouseY) {
        guiGraphics.drawString(this.font, Component.literal("RTG and Radiolysis Chamber"), 20, 4, -12829636, false);
        guiGraphics.drawString(this.font, Component.literal("Inventory"), 7, 71, -12829636, false);
        int x = (width - imageWidth) / 2;
        int y = (height - imageHeight) / 2;

        renderEnergyAreaTooltips(guiGraphics, mouseX, mouseY, x, y);
        renderSpecialOutputTooltip(guiGraphics, mouseX, mouseY, 61, 20, menu.getFluidStack());
    ...
    private void assignFluidRenderer() {
        renderer = new FluidTankRenderer(640000, true, 8, 61);
    }
    ...
    private void assignEnergyInfoArea() {
        ...
        energyInfoArea = new EnergyInfoArea(x + 8, y + 17, menu.blockEntity.getEnergyStorage(), 16, 34);
    }
    ...
    private void renderSpecialOutputTooltip(GuiGraphics graphics, int mx, int my, int offsetX, int offsetY, FluidStack stack) {
        if(isMouseAboveArea(mx, my, this.leftPos, this.topPos, offsetX, offsetY, 8, 48)) {
    ...
    private void renderEnergyAreaTooltips(GuiGraphics pGuiGraphics, int pMouseX, int pMouseY, int x, int y) {
        if(isMouseAboveArea(pMouseX, pMouseY, x, y, 8, 17, 16, 34)) {
