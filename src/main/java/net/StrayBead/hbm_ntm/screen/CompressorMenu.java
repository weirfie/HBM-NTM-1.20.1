...
public class CompressorMenu extends AbstractContainerMenu {
    ...
    public CompressorMenu(int id, Inventory inv, BlockEntity entity, ContainerData data) {
        super(NTMModMenus.COMPRESSOR_MENU.get(), id);
        ...
        this.blockEntity.getCapability(ForgeCapabilities.ITEM_HANDLER).ifPresent(handler -> {
            this.addSlot(new SlotItemHandler(handler, 0, 152, 69));
            this.addSlot(new SlotItemHandler(handler, 1, 62, 68));
            this.addSlot(new SlotItemHandler(handler, 2, 80, 68));
            this.addSlot(new SlotItemHandler(handler, 3, 26, 68));
        });
    ...
    private void addPlayerInventory(Inventory inv) {
        for (int si = 0; si < 3; ++si)
            for (int sj = 0; sj < 9; ++sj)
                this.addSlot(new Slot(inv, sj + (si + 1) * 9, 8 + sj * 18, 11 + 84 + si * 18));

    }

    private void addPlayerHotbar(Inventory inv) {
        for (int si = 0; si < 9; ++si)
            this.addSlot(new Slot(inv, si, 8 + si * 18, 11 + 142));
    }
    ...
    public int getSelectedPressure() {
