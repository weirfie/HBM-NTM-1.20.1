...
public class IronCrateMenu extends AbstractContainerMenu {
    ...
    public IronCrateMenu(int id, Inventory inv, FriendlyByteBuf extraData) {
        this(id, inv, inv.player.level().getBlockEntity(extraData.readBlockPos()), new SimpleContainerData(2));
    }

    public IronCrateMenu(int id, Inventory inv, BlockEntity entity, ContainerData data) {
        super(NTMModMenus.IRON_CRATE_MENU.get(), id);
        ...
        this.blockEntity.getCapability(ForgeCapabilities.ITEM_HANDLER).ifPresent(handler -> {
            ...
            for (int row = 0; row < 4; ++row) {
                for (int col = 0; col < 9; ++col) {
                    this.addSlot(new SlotItemHandler(handler, slotIndex, 8 + col * 18, (row * 18) + yOffset));
                    slotIndex++;
                }
            }
        });
    ...
    public boolean stillValid(Player player) {
        return stillValid(ContainerLevelAccess.create(level, blockEntity.getBlockPos()), player, ModBlocks.IRON_CRATE.get());
    }
